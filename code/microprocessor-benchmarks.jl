...
# ╔═╡ f7078e2c-7015-11ee-3005-cff6d36b48ba
begin
	using Gadfly
	using DataFrames
...
# ╔═╡ 2c5ca744-649b-4ed8-a2b9-b8fd030d6f12
begin
	cores = readdlm("../data/cores.dat", Float64, comments=true)
	frequency = readdlm("../data/frequency.dat", Float64, comments=true)
	specint = readdlm("../data/specint.dat", Float64, comments=true)
	transistors = readdlm("../data/transistors.dat", Float64, comments=true)
	watts = readdlm("../data/watts.dat", Float64, comments=true)
end

# ╔═╡ 1adc6f7e-e4f0-4c4f-92be-96e6dbac80c8
df = vcat(
...
# ╔═╡ a0ac4dfa-47c1-4e19-b1c4-9afd52d19175
p_proc = Gadfly.plot(df,
	x = :Year,
	y = :y,
	size = [0.5mm],
	color = :label,
	Scale.y_log10,
	Guide.title("Microprocessor metrics"),
	Guide.ylabel(nothing),
	Guide.colorkey(title=""),
	Coord.cartesian(ymin = -1, ymax = 8, xmin = 1970, xmax = 2023),
	Geom.point,
	Geom.smooth,
)

# ╔═╡ ef5359c4-a116-4d7c-b0f5-8a9994debf64
p_proc |> SVG("../images/microprocessors.svg", 15cm, 9cm)

# ╔═╡ a2c23334-53c6-4424-8d46-e96f917a21d7
begin
	data = CSV.read("../data/benchmarks.csv", DataFrame; header=["language", "benchmark", "time"])
	...
	#transform!(benchmarks, :language => col -> map(x -> dict[x], col) => :language)
	data[!,:language] = [dict[lang] for lang in data[!,:language]]
	data
end

# ╔═╡ 8dcfa96b-ada3-4fdd-929c-111f5a72818d
begin
	# Normalize benchmark times by C times
	ctime = data[data[!,:language].== "C", :]
	df_benchmarks = innerjoin(data, ctime, on=:benchmark, makeunique=true)
	select!(df_benchmarks, Not(:language_1)) # delete column language_1
	rename!(df_benchmarks, :time_1 =>:ctime)
	df_benchmarks[!,:normtime] = df_benchmarks[!,:time] ./ df_benchmarks[!,:ctime];
end

# ╔═╡ a0cbf3ef-97a9-403d-a9f0-dc1560d6ba86
begin
	...
	for lang in values(dict)
    	data = df_benchmarks[df_benchmarks[!,:language].== lang, :]
...
# ╔═╡ 4fb5b3d1-33ad-47cb-8b74-d29406a9a360
begin
	...
	benchmarks_ = innerjoin(df_benchmarks, langmean, on=:language)
	sort!(benchmarks_, [:priority, :geomean]);
	sort!(langmean,   [:priority, :geomean]);
end

# ╔═╡ 32ee76d0-4e63-4b6e-95aa-2552d903d6c8
begin
	p_benchmarks = Gadfly.plot(benchmarks_,
    	x = :language,
	    y = :normtime,
	    color = :benchmark,
	    Scale.y_log10,
...
# ╔═╡ 2dfa8238-5328-4f7a-b3ad-ae382f7ec5c7
p_benchmarks |> SVG("../images/benchmarks.svg", 17cm, 9cm)
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "d6bd17958190578d65e2ed8e8ab84c4f3210d5ec"
